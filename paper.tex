...
We compare the algorithm to the standard \emph{Ratio-Match} \cite{lowe2004sift} as well as the newer \emph{Mirror-Match} \cite{arnfred2013mirror}. In addition we compare the speed of \emph{Fast-Match} to three geometric algorithms; Adaptive Multi-Affing matching (\emph{AMA}) by Suiza et al.~\cite{puerto2011adaptive}, Local shape estimation by Del bimbo et. al (Del Bimbo)\cite{del2010local} and the Isomatch matching method by Das et al\cite{das2008event}. Because of the additional time needed to run the geometric algorithms we do not include them in comparisons featuring 3D objects and partially overlapping crops. However we compare \emph{AMA} to \emph{Fast-Match} and \emph{Ratio-Match} in a more limited survey featuring the same 4 image pairs used for the partially overlapping crops since this algorithm is the fastest according to our results and outperforms \emph{Del Bimbo} according to Suiza et al.~\cite{puerto2011adaptive}.

\emph{Fast-Match} with three matching algorithms; 
...
\begin{table}[htb]
    \caption{\emph{Fast-Match} and \emph{Ratio-Match} compared with \emph{AMA} on 4 image pairs from the Graffiti dataset by Mikolajczyk et al.\cite{mikolajczyk2005performance} as pictured in Figure~\ref{fig:patch_objects}. \emph{Fast-Match} and \emph{Ratio-Match} are evaluated with a threshold of $\tau = 0.6$ while \emph{AMA} is used with default parameters according to the reference implementation\protect\footnotemark. Precision is listed first with number of correct matches in parenthesis.}
\label{table:match}
	\centering
	\small
\begin{tabular}{r*{6}{r}}
    & \multicolumn{2}{c}{\emph{Fast-Match}} &\multicolumn{2}{c}{\emph{Ratio-Match}} & \multicolumn{2}{c}{\emph{AMA}} \\
\hline
\noalign{\smallskip}
Bark 1-4: & 0.99 & (1091) & 0.99 & (645) & 0.83 & (260)  \\
Boat 1-4: & 0.99 &(672) & 0.97 & (455) & 0.97 & (402) \\
Graf 1-3: & 0.67 &(192) & 0.72 & (141) & 0.83 & (610) \\
Wall 1-6: & 0.00 & (0) & 0.00 & (0) & 0.00 & (0) \\
\noalign{\smallskip}
\hline
\end{tabular}
\end{table}
\footnotetext{As found here: \url{http://ranger.uta.edu/~gianluca/feature\_matching/}}

% Ratio-Match
%({'bark': {'precision': 0.9984504667949431, 'recall': 645},
%  'boat': {'precision': 0.9660276729773398, 'recall': 455},
%  'graf': {'precision': 0.7157324074497083, 'recall': 141},
%  'wall': {'precision': 0.0, 'recall': 0}},

% Fast-Match
% {'bark': {'precision': 0.999999083410556, 'recall': 1091},
%  'boat': {'precision': 0.988233840832587, 'recall': 672},
%  'graf': {'precision': 0.668987216072418, 'recall': 192},
%  'wall': {'precision': 0.0, 'recall': 0}},

% AMA
% {'bark': {'precision': 0.8254, 'recall': 260},
%  'boat': {'precision': 0.9677, 'recall': 402},
%  'graf': {'precision': 0.8311, 'recall': 610},
%  'wall': {'precision': 0, 'recall': 0}})
...
\begin{figure}[tb]
    ...
    \begin{subfigure}[t]{1\columnwidth}
        \caption{3 Megapixels}
    \end{subfigure}
    \vspace{-0.5 mm}
    \begin{subfigure}[t]{0.49\columnwidth}
        \centering
        \includegraphics[width=1\columnwidth]{images/timings_medium}
    \end{subfigure}%
    ~ %
    \begin{subfigure}[t]{0.5\columnwidth}
        \centering
        \includegraphics[width=1\columnwidth]{images/timings_geom_medium}
    \end{subfigure}%
    \vspace{-2.5 mm}
    \begin{subfigure}[t]{1\columnwidth}
        \caption{10 Megapixels}
    \end{subfigure}
    \vspace{-0.5 mm}
    \begin{subfigure}[t]{0.49\columnwidth}
        \centering
        \includegraphics[width=1\columnwidth]{images/timings_large}
    \end{subfigure}%
    ~
    \begin{subfigure}[t]{0.5\columnwidth}
        \centering
        \includegraphics[width=1\columnwidth]{images/timings_geom_large}
    \end{subfigure}%
    \vspace{1.5 mm}
    \begin{subfigure}[t]{1\columnwidth}
        \centering
        ~\includegraphics[width=1\columnwidth]{images/timings_legend}
    \end{subfigure}%
    
    \caption{The duration in seconds the two versions of \emph{Fast-Match} compared with \emph{Ratio-Match} and the geometric matching algorithms \emph{AMA}, \emph{Isomatch} and \emph{Del Bimbo}. The two top plots shows the duration in seconds for matching two 3~MPixel images. The bottom plots shows the duration in seconds for matching two 10~Mpixel images. For the plots on the left, the first two results of each plot presume a precached image while the rest do not require knowing any inputs beforehand. The two plots on the right shows the timings for the precached and general version of \emph{Fast-Match} identical to the plots on the left but this time compared with three geometric matching algorithms. Note that the y-scale is different on all plots. The image pairs used can be seen in Figure~\ref{fig:match_example}}
    \label{fig:timings}
\end{figure}

Table~\ref{table:match} lists the precision and number of correct matches from matching $4$ image pairs as seen in Figure~\ref{fig:patch_objects}. The geometric matching method \emph{AMA} yields better results than \emph{Fast-Match} and \emph{Ratio-Match} for the more challenging `Graf' image pair while all algorithms yield no correct matches on the `Wall' pair. For the more trivial `Boat' and `Bark' images we find \emph{Fast-Match} performing similar to \emph{Ratio-Match} in terms of precision while returning markedly more correct matches.

Figure~\ref{fig:timings} demonstrate the speed of \emph{Fast-Match} on image sizes as is typically produced by modern consumer cameras and camera phones. We compare \emph{Fast-Match} with different variations of \emph{Ratio-Match} and three geometric matching algorithms on two image pairs, one with two images of 10~Mpixels and another where the same image pair has been scaled to 3~Mpixels. The differences between \emph{Fast-Match} and \emph{Ratio-Match} are largest for larger images. The retrieval variant of \emph{Fast-Match} matches the image pairs in around one second while a retrieval variant of \emph{Ratio-Match} with precomputed features spends eight seconds on the same task. This result is roughly equal to the time the general variation of \emph{Fast-Match} spends matching the two images without any pre-computations. For the nearest neighbor search we note that for large images there are substantial speed gains to be had by choosing the right algorithm for finding nearest neighbors, with Muja et al.'s Flann matcher being vastly superior to brute force and KGraph. For the 3 megapixel picture the pattern repeats itself, although with smaller margins separating \emph{Fast-Match} and \emph{Ratio-Match}. When images are smaller, computations like obtaining the initial seed matches are comparatively more costly.
    
    In the plots featuring geometric matching algorithms we compare \emph{Fast-Match} with three matching algorithms; Adaptive Multi-Affing matching (AMA) by Suiza et al.~\cite{puerto2011adaptive}, Local shape estimation by Del bimbo et. al (Del Bimbo)\cite{del2010local} and the Isomatch matching method by Das et al\cite{das2008event}. For both the 3~Mpixel and 10~Mpixel images the differences between \emph{Fast-Match} and the geometric algorithms in terms of speed is vast. For the 3~Mpixel images the general \emph{Fast-Match} method is roughly $40$ times faster than the closest competitor, \emph{AMA} while it is more then $50$ times as fast as \emph{AMA} for the 10~Mpixel images. While both \emph{Isomatch} and \emph{AMA} are slow they scale well with the increased image sizes. \emph{Del Bimbo} on the other hand spends more than an hour finding matches for the 10~Mpixel image pairs.
